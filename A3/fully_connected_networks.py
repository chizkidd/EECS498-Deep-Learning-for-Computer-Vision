...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # Reshape input x to have shape (N, D)
        N = x.shape[0]
        x_reshaped = x.reshape(N, -1)  # Flatten all but the first dimension
        
        # Compute the forward pass
        out = x_reshaped @ w + b  # Matrix multiplication and bias addition
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        x, w, b = cache
        N = x.shape[0]
        x_reshaped = x.reshape(N, -1)  # Flatten input for gradient calculations
        
        # Compute gradients
        dw = x_reshaped.T @ dout  # Gradient with respect to weights
        db = dout.sum(axis=0)  # Gradient with respect to biases
        dx = dout @ w.T  # Gradient with respect to input
...
class ReLU(object):
    ...
    def forward(x):
        ...
        # out = torch.maximum(torch.tensor(0.0, device=x.device), x)  # Apply ReLU activation
    ...
    def backward(dout, cache):
        ...
        dx = dout * (x > 0).float()  # Gradient is passed only for positive x values
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        assert b is not None, "b is None"
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # Debugging prints
        print(f"W1 initialized: {self.params['W1'].shape}, dtype: {self.params['W1'].dtype}")
        print(f"W2 initialized: {self.params['W2'].shape}, dtype: {self.params['W2'].dtype}")
    ...
    def loss(self, X, y=None):
        ...
        assert b1 is not None, "b1 is None"
        assert W2 is not None, "W2 is None"
        assert b2 is not None, "b2 is None"

        # Forward pass
        hidden, cache_hidden = Linear_ReLU.forward(X, W1, b1)
        scores, cache_scores = Linear.forward(hidden, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss = torch.nn.functional.cross_entropy(scores, y.to(self.device))
        loss += self.reg * (torch.sum(W1**2) + torch.sum(W2**2))  # L2 regularization

        grads = {}
        dscores = torch.nn.functional.softmax(scores, dim=1)
        dscores[range(X.shape[0]), y] -= 1
        dscores /= X.shape[0]

        dhidden, grads['W2'], grads['b2'] = Linear.backward(dscores, cache_scores)
        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(dhidden, cache_hidden)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            out, cache[f'fc{i+1}'] = Linear_ReLU.forward(out, W, b)
            if self.use_dropout:
                out, cache[f'dropout{i+1}'] = Dropout.forward(out, self.dropout_param)
        # Output/Final layer (no ReLU or dropout)
        W = self.params[f'W{self.num_layers}']
        b = self.params[f'b{self.num_layers}']
        scores, cache[f'fc{self.num_layers}'] = Linear.forward(out, W, b)
        ...
        loss, dscores = self._softmax_loss(scores, y)
        for i in range(self.num_layers):
            W = self.params[f'W{i+1}']
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dout, grads[f'W{i}'], grads[f'b{i}'] = Linear.backward(dout, cache[f'fc{i}'])
            else:
                if self.use_dropout:
                    dout = Dropout.backward(dout, cache[f'dropout{i}'])
                dout, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dout, cache[f'fc{i}'])
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Generate a binary mask with probability p of dropping
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            # Apply the same mask and scaling as in the forward pass
            dx = dout * mask / (1.0 - dropout_param['p'])
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
        elif mode == 'test':
