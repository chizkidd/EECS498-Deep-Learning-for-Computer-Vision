...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        ...
        # dists[i, j] = torch.sum((x_train[i].flatten() - x_test[j].flatten()) ** 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      dists[i] = torch.sum((x_train[i] - x_test) ** 2, dim=tuple(range(1, x_train.ndim)))
      # dists[i, :] = torch.sum((x_train[i].unsqueeze(0) - x_test)**2, dim=(1,2,3))
      # dists[i] = torch.sum((x_test.flatten(start_dim=1) - x_train[i].flatten()) ** 2, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train, -1)   # shape (num_train, C*H*W)
    x_test_flat = x_test.view(num_test, -1)      # shape (num_test, C*H*W)

    # Use matrix multiplication to compute distances efficiently:
    # (a-b)^2 = a^2 - 2ab + b^2
    # Where a is x_train and b is x_test
    
    # Compute a^2: sum of squares for each training example
    train_square = (x_train_flat**2).sum(dim=1, keepdim=True)
    
    # Compute b^2: sum of squares for each test example 
    test_square = (x_test_flat**2).sum(dim=1)
    
    # Compute -2ab using matrix multiplication
    # (num_train x D) @ (D x num_test) -> (num_train x num_test)
    cross_term = -2 * x_train_flat @ x_test_flat.t()
    
    # Add the terms using broadcasting
    dists = train_square + cross_term + test_square
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        # Get indices of k nearest neighbors by finding k smallest distances
        # dists[:,i] gets distances from all training points to test point i
        # largest=False means we want smallest distances (nearest neighbors)
        topk_indices = torch.topk(dists[:, i], k, dim=0, largest=False).indices
        
        # Get most common label among k nearest neighbors
        # torch.mode finds the most frequent value
        # .values extracts just the value rather than both value and count
        y_pred[i] = torch.mode(y_train[topk_indices]).values
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            # Create training set by concatenating all folds except the current one
            ...
            # Create validation set using the current fold
            x_val_fold = x_train_folds[i]
            y_val_fold = y_train_folds[i]
            # Create classifier with training set
            classifier = KnnClassifier(x_train_fold, y_train_fold)
            # Check accuracy on validation set
            accuracy = classifier.check_accuracy(x_val_fold, y_val_fold, k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Find k with highest mean accuracy by using max() with a key function that computes
    # the mean accuracy for each k (sum of accuracies divided by number of folds)
