...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(start, stop+1):
        if i % 10 == 0:
            y.append(i)
    x = torch.tensor(y, dtype=torch.float64) 

    #x = torch.tensor([x for x in range(start, stop + 1) if x % 10 == 0], dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    C = max(x) + 1
    y = torch.eye(C)[x].to(torch.float32)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Get column indices of minimum elements in each row
    col_indices = torch.argmin(y, dim=1)  

    # Set the minimum elements to 0 using advanced indexing
    y[row_indices, col_indices] = 0  
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    all_values = torch.cat(xs)
    
    # Compute cumulative sum
    cumsum = torch.cumsum(all_values, dim=0)
    
    # Compute means using cumulative sums and lengths
    total_sums = cumsum[torch.cumsum(ls, dim=0) - 1]
    total_sums = torch.cat([total_sums[:1], total_sums[1:] - total_sums[:-1]])
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Sort the tensor and keep track of original indices
    sorted_x, sorted_indices = x.sort()
    
    # Identify unique elements
    unique_mask = torch.cat([torch.tensor([True], device=x.device), sorted_x[1:] != sorted_x[:-1]])
    
    # Extract unique values and their first indices
