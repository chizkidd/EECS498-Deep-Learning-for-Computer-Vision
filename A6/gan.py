...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cpu"):
    ...
    ##############################################################################
    noise = torch.rand(batch_size, noise_dim, dtype=dtype, device=device) * 2.0 - 1.0
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
...
def discriminator_loss(logits_real, logits_fake):
    ...
    #############################################################################
    # Discriminator wants to output 1 for real images and 0 for fake images
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ##############################################################################
    # Loss = 0.5 * E[(D(x) - 1)^2] + 0.5 * E[D(G(z))^2]
    # LSGAN discriminator: minimize (scores_real - 1)^2 + (scores_fake - 0)^2
...
def ls_generator_loss(scores_fake):
    ...
    ##############################################################################
    # Loss = 0.5 * E[(D(G(z)) - 1)^2]
    # LSGAN generator: minimize (scores_fake - 1)^2
...
def build_dc_classifier():
    ...
    model = nn.Sequential(
        ...
        nn.MaxPool2d(kernel_size=2, stride=2),
        nn.Flatten(),
        nn.Linear(1024, 1024), # output size: 4x4x64
        nn.LeakyReLU(0.01),
        nn.Linear(1024, 1)
    )
    ############################################################################
