...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        # Layers to compute mu and logvar from hidden features
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Sigmoid(),
            nn.Unflatten(1, (1, 28, 28))  # Assuming 28x28 images
        )
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # (1) Pass input through encoder to get hidden features
        hidden = self.encoder(x)
        
        # Get posterior mu and logvar
        ...
        # (3) Pass z through decoder to reconstruct x
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        # Layers to compute mu and logvar
        self.mu_layer = nn.Linear(self.hidden_dim, latent_size)
        self.logvar_layer = nn.Linear(self.hidden_dim, latent_size)
    ...
    def forward(self, x, c):
        ...
        #     resconstruct x                                                      #
        ###########################################################################
        # (1) Flatten x and concatenate with one-hot class vector c
        x_flat = x.view(x.size(0), -1)  # (N, H*W)
        encoder_input = torch.cat([x_flat, c], dim=1)  # (N, H*W + C)
        ...
        # (3) Concatenate z with class vector and decode
        decoder_input = torch.cat([z, c], dim=1)  # (N, Z + C)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # notebook                                                                    #
    ###############################################################################
    # Reconstruction loss (binary cross-entropy) (summed over spatial dimensions)
