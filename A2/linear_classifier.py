    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                dW[:, j] += X[i]      # gradient for the incorrect class
                dW[:, y[i]] -= X[i]   # gradient for the correct class
                #######################################################################
                #                       END OF YOUR CODE                              #
                #######################################################################
...
def svm_loss_vectorized(
    ...
    scores = X.matmul(W)  # N x C
    correct_class_scores = scores[range(num_train), y].view(-1, 1)  # N x 1
    margins = torch.maximum(scores - correct_class_scores + 1, torch.zeros_like(scores))  # N x C
    margins[range(num_train), y] = 0  # ignore the correct class
    ...
    # Replace "pass" statement with your code
    binary = margins > 0
    binary = binary.float()  # Convert to float to allow the assignment operation
    binary_sum = torch.sum(binary, dim=1)
    binary[range(num_train), y] = -binary_sum

    # Ensure binary has the same dtype as X.t()
    binary = binary.type(X.dtype)  
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [5e-3, 1e-3, 1e-2, 1e-1]
...
def test_one_param_set(
    ...
    train_acc = (cls.predict(X_train) == y_train).float().mean().item()
    val_acc = (cls.predict(X_val) == y_val).float().mean().item()
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = X[i].reshape(1, -1) @ W  # (1, C)
        # shift the values of f so that the highest number is 0:
        scores -= torch.max(scores)
        # calculate the probabilities
        prob = torch.exp(scores) / torch.sum(torch.exp(scores))  # (1, C)
        # calculate loss
        loss += -torch.log(prob[0, y[i]])

        # calculate gradient
        for j in range(num_classes):
            # Convert boolean to float for subtraction
            dW[:, j] += (prob[0, j] - (j == y[i]).type(X.dtype)) * X[i]  # gradient for each class
...
def softmax_loss_vectorized(
    ...
    loss = -torch.log(probs[torch.arange(num_train), y]).mean()
    loss += reg * torch.sum(W * W)

    # 1) one way to calculate dW (one-hot encoding)
    # Create one-hot matrix for true labels
    y_one_hot = torch.zeros_like(probs)
    y_one_hot[torch.arange(num_train), y] = 1
    
    # Compute gradient
    dW = X.t() @ (probs - y_one_hot)
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1e-5, 1e-3, 1e-1, 1e1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
