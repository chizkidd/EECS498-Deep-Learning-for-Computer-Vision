...
def nn_forward_backward(
    ...
    loss = correct_logprobs.mean() + reg * (torch.sum(W1**2) + torch.sum(W2**2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-4, 1e-2, 0]
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    net = TwoLayerNet(X_train.shape[1], hidden_size, num_classes)
