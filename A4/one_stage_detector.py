...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_ch in stem_channels:
            # Cls Stem
            conv_cls = nn.Conv2d(curr_channels, out_ch, kernel_size=3, stride=1, padding=1)
            nn.init.normal_(conv_cls.weight, std=0.01)
            nn.init.constant_(conv_cls.bias, 0)
            stem_cls.extend([conv_cls, nn.ReLU(inplace=True)])
            
            # Box Stem
            conv_box = nn.Conv2d(curr_channels, out_ch, kernel_size=3, stride=1, padding=1)
            nn.init.normal_(conv_box.weight, std=0.01)
            nn.init.constant_(conv_box.bias, 0)
            stem_box.extend([conv_box, nn.ReLU(inplace=True)])
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, x in feats_per_fpn_level.items():
            # Apply stems
            cls_feat = self.stem_cls(x) # classification stem
            box_feat = self.stem_box(x) # box regression/centerness stem

            # Predict and permute to (B, H*W, C) [(B, C, H, W) -> (B, H, W, C) -> (B, H*W, C)]
            class_logits[level] = self.pred_cls(cls_feat).permute(0, 2, 3, 1).flatten(1, 2)
            boxreg_deltas[level] = self.pred_box(box_feat).permute(0, 2, 3, 1).flatten(1, 2)
            centerness_logits[level] = self.pred_ctr(box_feat).permute(0, 2, 3, 1).flatten(1, 2)
...
def fcos_get_deltas_from_locations(
    ...
    # Extract xc, yc from locations (N, 2)
    xc = locations[:, 0]
    yc = locations[:, 1]

    # Extract x1, y1, x2, y2 from gt_boxes (N, 4 or 5)
    x1 = gt_boxes[:, 0]
    y1 = gt_boxes[:, 1]
    x2 = gt_boxes[:, 2]
    y2 = gt_boxes[:, 3]
    ...
    # Stack into (N, 4) and normalize by stride
    deltas = torch.stack([l, t, r, b], dim=1) / stride

    # Handle background boxes: if any GT coordinate is -1, 
    # the entire delta for that location should be -1.
    # We can check if x1 == -1.
    is_background = (x1 == -1)
...
def fcos_apply_deltas_to_locations(
    ...
    # 1. Un-normalize the deltas by multiplying with stride
    # 2. Clip to zero to ensure the center stays inside the resulting box
    deltas = deltas.clamp(min=0) * stride

    # Extract xc, yc from locations
    xc, yc = locations[:, 0], locations[:, 1]
    
    # Extract l, t, r, b from un-normalized deltas
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Extract l, t, r, b ----> deltas: (N, 4) -> L, T, R, B
    l, t, r, b = deltas[:, 0], deltas[:, 1], deltas[:, 2], deltas[:, 3]

    # Compute the horizontal and vertical centerness components
    # We use min/max to ensure the value is between 0 and 1

    # Formula: sqrt( (min(l,r)/max(l,r)) * (min(t,b)/max(t,b)) )
    horizontal_LR = torch.min(l, r) / (torch.max(l, r) + 1e-6)
    vertical_TB = torch.min(t, b) / (torch.max(t, b) + 1e-6)
    centerness = torch.sqrt(horizontal_LR * vertical_TB) # combined centerness target
    
    # Handle background
    # If any delta is negative, the location is outside a valid GT box
    is_background = torch.any(deltas < 0, dim=1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        shapes = {k: v.shape for k, v in fpn_feats.items()}
        locations_per_fpn_level = get_fpn_location_coords(shapes, self.backbone.fpn_strides, images.dtype, images.device)
        ...
        # Iterate over batch to match locations to GT
        for i in range(images.shape[0]):
            # Get matched boxes for this image
            img_matched_gt = fcos_match_locations_to_gt(
                locations_per_fpn_level, self.backbone.fpn_strides, gt_boxes[i]
            )
            matched_gt_boxes.append(img_matched_gt)
            
            # Compute deltas for this image across all levels
            img_gt_deltas = {}
            for level, locs in locations_per_fpn_level.items():
                img_gt_deltas[level] = fcos_get_deltas_from_locations(
                    locs, img_matched_gt[level], self.backbone.fpn_strides[level]
                )
            matched_gt_deltas.append(img_gt_deltas)
        ...
        # Positive mask for regression and centerness
        pos_mask = (matched_gt_boxes[:, :, 4] != -1)

        # Classification Loss: Focal Loss
        # Create one-hot targets; background pixels are all zeros
        gt_classes = matched_gt_boxes[:, :, 4].long()
        target_classes = F.one_hot(gt_classes.clip(min=0), self.num_classes).float()
        target_classes[gt_classes < 0] = 0
        ...
        # Flatten for target calculation, then reshape back
        flat_deltas = matched_gt_deltas.view(-1, 4)
        gt_ctr = fcos_make_centerness_targets(flat_deltas).view(pred_ctr_logits.shape[:2])
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.squeeze(-1), gt_ctr, reduction="none"
        )
        loss_ctr[~pos_mask] = 0
        ...
        # Ensure normalizer is at least 1.0 to avoid division by zero/tiny values
        # images.shape[0] is the current batch size
        batch_size = images.shape[0]
        norm_factor = max(1.0, self._normalizer) * batch_size

        return {
            "loss_cls": loss_cls.sum() / norm_factor,
            "loss_box": loss_box.sum() / norm_factor,
            "loss_ctr": loss_ctr.sum() / norm_factor,
        }
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Compute geometric mean of class probabilities and centerness
            # We use sigmoid to turn logits into [0, 1] probabilities
            prob_cls = level_cls_logits.sigmoid()
            prob_ctr = level_ctr_logits.sigmoid()
            level_pred_scores = torch.sqrt(prob_cls * prob_ctr)

            # Step 1: Get the most confidently predicted class and its score
            # Shape: (N, num_classes) -> (N,), (N,)
            level_pred_scores, level_pred_classes = level_pred_scores.max(dim=1)

            # Step 2: Retain predictions above the threshold
            ...
            # Filter deltas and locations for the next steps
            filtered_deltas = level_deltas[keep_idx]
            filtered_locations = level_locations[keep_idx]

            # Step 3: Obtain predicted boxes using implemented function
            # We use the un-normalization logic built into fcos_apply_deltas_to_locations
            # Shape: (N, 4)
            ...
            # Step 4: Clip XYXY box-coordinates to image dimensions
