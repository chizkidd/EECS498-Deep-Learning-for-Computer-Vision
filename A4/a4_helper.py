...
class VOC2007DetectionTiny(torch.utils.data.Dataset):
    ...
    def _attempt_download(dataset_dir: str):
        ...
        # 1. Download to LOCAL first (Fastest)
        tar_filename = "VOCtrainval_06-Nov-2007.tar"
        local_tar = os.path.join("/content", tar_filename)
        final_tar_on_drive = os.path.join(dataset_dir, tar_filename)

        if not os.path.exists(final_tar_on_drive):
            print(f"Downloading {tar_filename} to local SSD...")
            with requests.get(f"https://web.eecs.umich.edu/~justincj/data/{tar_filename}", stream=True, verify=False) as r:
                with open(local_tar, 'wb') as f:
                    shutil.copyfileobj(r.raw, f)
            print("Moving zip to Drive for backup...")
            shutil.copy(local_tar, final_tar_on_drive)
        else:
            print("Tar exists on Drive. Copying to local SSD for fast extraction...")
            shutil.copy(final_tar_on_drive, local_tar)

        # 2. Extract LOCALLY (Takes seconds vs. hours on Drive)
        local_extract_path = "/content/VOCdevkit_local"
        if not os.path.exists(local_extract_path):
            print("Extracting locally...")
            with tarfile.open(local_tar) as tar:
                tar.extractall(path="/content")
            # Usually extracts to /content/VOCdevkit
            os.rename("/content/VOCdevkit", local_extract_path)

        # 3. Use System Shell to push to Drive (Much more robust)
        drive_devkit = os.path.join(dataset_dir, "VOCdevkit")
        if not os.path.exists(drive_devkit):
            print("Pushing extracted files to Drive. This is the only slow part left...")
            # Using shell 'cp -r' is often faster than shutil.copytree for Drive
            subprocess.run(["cp", "-r", local_extract_path, drive_devkit])
            print("Extraction and Sync complete!")
        else:
            print("Dataset already present on Drive.")
            
        # Clean up local space
        if os.path.exists(local_tar): os.remove(local_tar)

    @staticmethod
    def _test_attempt_download(dataset_dir: str):
        """
...
def train_detector(
    ...
    for _iter in range(max_iters):
        ...
        # Check for NaN before backward to catch it early
        if torch.isnan(total_loss):
            print(f"NaN detected at iteration {_iter}. Stopping training.")
            break
        total_loss.backward()

        # Apply Gradient Clipping
        # max_norm=35 is a standard value for FCOS/RetinaNet to prevent explosion
        torch.nn.utils.clip_grad_norm_(detector.parameters(), max_norm=35.0)
