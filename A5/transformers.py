...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
      ...
      for j in range(M):
        weights[j] = torch.dot(query[i], key[j]) / (M**0.5)

      weights_softmax = torch.softmax(weights, dim=-1)
      out[i] = weights_softmax @ value # weighted sum of values: (1, K) @ (K, M) -> (1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            # Compute scaled dot product for one query vs all keys in the batch item
            # query[n, i] is (M,), key[n] is (K, M). Result is (K,)
            weights = (query[n, i] @ key[n].T) / math.sqrt(M)
            
            # Softmax across the key dimension
            weights_softmax = torch.softmax(weights, dim=-1)
            
            # Weighted sum: (K,) @ (K, M) -> (M,)
            out[n, i] = weights_softmax @ value[n]
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # 1. Calculate Q @ K.T (Batched)
    # Compute scaled dot product attention weights  (N, K, M) x (N, M, K) -> (N, K, K)
    ...
    # 3. Calculate the Softmax across the last dimension (keys) and then the Weighted sum of values
    weights_softmax = torch.softmax(scores, dim=-1)
    y = torch.bmm(weights_softmax, value)   # (N, K, K) x (N, K, M) -> (N, K, M)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        qkv_list = [self.q, self.k, self.v]
        for layer in qkv_list:
          d_in = layer.in_features
          d_out = layer.out_features
          c = math.sqrt(6.0 / (d_in + d_out))
    ...
    def forward(
        ...
        # Shape: q_proj -> (N, K, dim_q), k_proj -> (N, K, dim_q), v_proj -> (N, K, dim_v)
        q_proj, k_proj, v_proj = self.q(query), self.k(key), self.v(value)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(q_proj, k_proj, v_proj)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.out_proj = nn.Linear(num_heads * dim_out, dim_in)
    ...
    def forward(
        ...
        ##########################################################################
        head_out = [head(query, key, value, mask) for head in self.heads]  # (N, K, dim_out)
        out_concat = torch.cat(head_out, dim=-1)  # (N, K, num_heads × dim_out)

        y = self.out_proj(out_concat)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        out1 = self.MHA(x, x, x)
        norm1 = self.LN1(x + out1)
        out2 = self.dropout(norm1)
        out3 = self.FFN(out2)
        norm2 = self.LN2(out2 + out3)
        y = self.dropout(norm2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    #                                                                             #
    ###############################################################################
    N, K = seq.shape  # shape: batch_size, seq_len
    
    # # Create a lower triangular matrix (including diagonal)
    # # True values indicate positions that should be MASKED (i.e., future positions)
    # # The mask should be upper triangular (excluding diagonal) to mask future tokens
    # mask_2D = torch.triu(torch.ones(K, K, dtype=torch.bool, device=seq.device), diagonal=1)
    
    # # Expand to batch size: (K, K) -> (N, K, K)
    # mask = mask_2D.unsqueeze(0).expand(N, K, K)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        out1 = self.attention_self(dec_inp, dec_inp, dec_inp, mask=mask)
        norm1 = self.norm1(out1 + dec_inp)
        out2 = self.dropout(norm1)
        out3 = self.attention_cross(out2, enc_inp, enc_inp)
        norm2 = self.norm2(out2 + out3)
        out4 = self.dropout(norm2)
        out5 = self.feed_forward(out4)
        norm3 = self.norm3(out4 + out5)
        y = self.dropout(norm3)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # 1. Create 1D tensor [0/K, 1/K, ..., (K-1)/K]
    # Use torch.arange for the numerator and divide by K
    pos = torch.arange(K).float() / K
    
    # 2. Replicate this M times and reshape to (1, K, M)
    # .view(1, K, 1) prepares it for broadcasting, then .expand makes it (1, K, M)
    y = pos.view(1, K, 1).expand(1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # # Create a (K, M) matrix of zeros
    # pe = torch.zeros(K, M)
    
    # # Create a column vector for positions (0 to K-1)
    # position = torch.arange(K).unsqueeze(1).float()
    
    # # Compute the divisor term: 10000^(2i/M)
    # # Note: we only need to compute this for half the dimensions (i = 0, 1, ..., M/2)
    ...
    # # Reshape to (1, K, M) to match expected output
    # y = pe.unsqueeze(0)

    pos = torch.arange(K, dtype=torch.float32).reshape(K, 1)
    dim = torch.arange(M, dtype=torch.float32).reshape(1, M)
    
    # Calculate denominator
    denominator = torch.pow(10000.0, (2 * (dim // 2)) / M)
    
    # Apply sin to even indices, cos to odd
    y = torch.zeros(K, M)
    y[:, 0::2] = torch.sin(pos / denominator[:, 0::2])
    y[:, 1::2] = torch.cos(pos / denominator[:, 1::2])
...
class Transformer(nn.Module):
    def __init__(
        ...
        # Xavier-style initialization as used in previous blocks
        c = math.sqrt(6.0 / (vocab_len + emb_dim))
        nn.init.uniform_(self.emb_layer.weight, -c, c)
    ...
    def forward(
        ...
        enc_out = self.encoder(self.dropout(q_emb_inp))
        
        # Create mask for decoder
        mask = get_subsequent_mask(ans_b[:, :-1])
        
        # Pass through decoder
        dec_out = self.decoder(self.dropout(a_emb_inp), enc_out, mask)
        
        # Reshape output to (N*O, V)
        dec_out = dec_out.reshape(-1, self.vocab_len)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class AddSubDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        convert_str_to_tokens: dict[str, int],
