...
def rnn_backward(dh, cache):
    ...
    dx = torch.zeros(N, T, D, device=dh.device)
    dWx = torch.zeros_like(Wx)
    dWh = torch.zeros(H, H, device=dh.device)
    db = torch.zeros(H, device=dh.device)

    dprev_h = torch.zeros(N, H, device=dh.device)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.fc_h0 = nn.Linear(self.image_encoder.out_channels, hidden_dim)
        ...
        if cell_type == "rnn":
            ...
            self.fc_A = nn.Conv2d(self.image_encoder.out_channels, hidden_dim, 1)
    ...
    def forward(self, images, captions):
        ...
        h0 = self.fc_h0(pooled)

        x = self.word_embedding(captions_in)

        if self.cell_type == "rnn":
            h, _ = rnn_forward(x, h0, self.rnn.Wx, self.rnn.Wh, self.rnn.b)
    ...
    def sample(self, images, max_length=15):
        ...
        # prev_word = torch.full((N,), self._start, device=images.device)

        # for t in range(max_length):
        #     x = self.word_embedding(prev_word).unsqueeze(1)

        #     if self.cell_type == "rnn":
        #         h = self.rnn.step_forward(x.squeeze(1), h)
        #     else:
        #         h, c = self.rnn.step_forward(x.squeeze(1), h, c)
        ...
        for t in range(max_length):
            x = self.word_embedding(prev_word)
            ...
            if self.cell_type == "rnn":
                ...
                h, c, attn = self.rnn.step_forward(x, h, c, A)
                attn_weights_all[:, t] = attn
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        hn = torch.zeros(N, T, H, device=x.device)
...
def dot_product_attention(prev_h, A):
    ...
    scores = torch.einsum("nh,nhk->nk", prev_h, A_flat) / math.sqrt(H)
    weights = F.softmax(scores, dim=1)

    attn = torch.einsum("nk,nhk->nh", weights, A_flat)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # LSTM gates
        z = x @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b
        ...
        #                           END OF YOUR CODE                         #
        ######################################################################
        return next_h, next_c, attn_weights
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        h = torch.zeros(N, T, H, device=x.device) 
        prev_h = A.mean(dim=(2, 3)) # Initial hidden state, of shape (N, H)
        prev_c = prev_h  # Initial cell state, of shape (N, H)
