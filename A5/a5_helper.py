...
def inference2(model, inp_exp, inp_exp_pos, out_pos_exp, out_seq_len):
    model.eval()
    # Ensure y_init is on the correct device (GPU)
    y_init = torch.LongTensor([14]).unsqueeze(0).to(inp_exp.device) # Shape: (1, 1)

    with torch.no_grad(): # Disable gradients for speed and memory
        # 1. Encode the question once
        ques_emb = model.emb_layer(inp_exp)
        q_emb_inp = ques_emb + inp_exp_pos
        enc_out = model.encoder(q_emb_inp)
        
        for i in range(out_seq_len - 1):
            # 2. Embed the currently generated tokens
            ans_emb = model.emb_layer(y_init)
            # Match positional encoding length to y_init length
            current_pos = out_pos_exp[:, :y_init.shape[1], :]
            a_emb_inp = ans_emb + current_pos
            
            # 3. CRITICAL: Generate the causal mask for the current sequence
            # This ensures the decoder attends correctly to previous tokens
            mask = get_subsequent_mask(y_init)
            
            # 4. Decode
            dec_out = model.decoder(a_emb_inp, enc_out, mask)
            
            # 5. Get the prediction for the VERY LAST token generated
            # dec_out shape: (1, current_seq_len, vocab_len)
            last_token_logits = dec_out[:, -1, :] 
            next_word = torch.argmax(last_token_logits, dim=-1)

            # 6. Append to the sequence
            y_init = torch.cat([y_init, next_word.view(1, 1)], dim=1)
            
            # Optional: Stop early if EOS is predicted
            # if next_word.item() == EOS_TOKEN_ID: break
